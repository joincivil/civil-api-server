...
// VerifyEthSignature accepts an Ethereum address, a message string, and a signature and confirms that the signature was indeed signed by the address specified
...
// VerifyEthChallenge confirms that a "challenge" string has a timestamp that is within {gracePeriod} number of seconds from the current time
