...
// NewListingMapLoader creates a new ListingMapLoader given a fetch, wait, and maxBatch
func NewListingMapLoader(config ListingMapLoaderConfig) *ListingMapLoader {
	return &ListingMapLoader{
		fetch:    config.Fetch,
		wait:     config.Wait,
...
func (l *ListingMapLoader) Clear(key string) {
	l.mu.Lock()
	delete(l.cache, key)
	l.mu.Unlock()
}
