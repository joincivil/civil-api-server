...
func TestService(t *testing.T) {
	...
	t.Run("UpdateEtherPayment complete", func(t *testing.T) {
		...
		// create the payment
		p, err := paymentService.CreateEtherPayment(channelID, ownerType, ownerID.String(), payments.EtherPayment{TransactionID: tx.Hash().String()}, nil)
		if err != nil {
	...
	t.Run("UpdateEtherPayment complete with email", func(t *testing.T) {
		...
		// create the payment
		p, err := paymentService.CreateEtherPayment(channelID, ownerType, ownerID.String(), payments.EtherPayment{TransactionID: tx.Hash().String(), PaymentModel: payments.PaymentModel{EmailAddress: "nick@joincivil.com"}}, nil)
		if err != nil {
			t.Fatalf("not expecting error: %v", err)
		}
	...
	t.Run("UpdateEtherPayment complete with bad email", func(t *testing.T) {
		...
		// create the payment
		p, err := paymentService.CreateEtherPayment(channelID, ownerType, ownerID.String(), payments.EtherPayment{TransactionID: tx.Hash().String(), PaymentModel: payments.PaymentModel{EmailAddress: "243-2"}}, nil)
		if err != nil {
			t.Fatalf("not expecting error: %v", err)
		}
